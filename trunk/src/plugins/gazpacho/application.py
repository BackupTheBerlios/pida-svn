...
from gazpacho.bugreportdialog import BugReportDialog
...
class Application(object):
    ...
    def __init__(self):
        ...
        self._container = self._application_window_create()
...
"""
  ...
  <menubar name="MainMenu">
    ...
    # some properties
    def get_window(self): return self._container
    window = property(get_window)

    def get_container(self): return self._container
    ...
    def _refresh_title(self):
        if self.get_window() is self.get_container():
            if self._project:
    ...
    def _open_cb(self, action):
        filename = dialogs.open(parent=self.get_window(), patterns=['*.glade'],
    ...
    def _confirm_close_projects(self, projects):
        ...
        if len(projects) == 1:
            dialog = SingleCloseConfirmationDialog(projects[0], self.get_window())
        else:
            dialog = MultipleCloseConfirmationDialog(projects, self.get_window())
    ...
    def _create_debugging_window(self, view, title, action):
        win = gtk.Window()
        win.set_title(title)
        win.set_transient_for(self.get_window())
        win.add(view)
        view.show_all()
    ...
    def _show_clipboard_cb(self, action):
        ...
        if self._clipboard_window is None:
            ...
            self._clipboard_window = ClipboardWindow(self.get_window(),
                                                     self._clipboard)
    ...
    def send_bugreport(self, exception_text, parent_window=None):
        ...
        """
        d = BugReportDialog(parent_window)
        r = d.run()
        retval = False
        if r == gtk.RESPONSE_ACCEPT:
            params = {}
            params['description'] = d.get_description()
            params['exception'] = exception_text
            params['name'] = d.get_name()
            params['from'] = d.get_email()
            today = time.strftime("%Y-%m-%d %H:%M")
            params['subject'] = 'Crash on %s' % today
            params['platform'] = sys.platform
            params['python'] = sys.version
